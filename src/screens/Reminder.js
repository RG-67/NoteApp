...
import { getReminderNotes } from "../api/noteApi";
...
function Reminder({navigation}) {
    ...
    const getRmNotes = async () => {
        try {
            const {databaseUserId, userId} = await getCredentials();
            const response = await getReminderNotes(databaseUserId, userId);
            ...
            console.error("getRmNotesErr ==>", error);
        }
    }
